...
def test_basic_operations():
    ...
    # Initialize Range ORAM with 16 blocks, bucket size 4, and range size 4
    num_blocks = 16
...
def test_batch_eviction():
    ...
    # Initialize with smaller numbers to trigger batch eviction more quickly
    roram = RangeORAM(numBlocks=8, bucketSize=4, range_size=2)
    server = Server(roram.height, roram.bucketSize)
    roram.connectServer(server)
    roram.initializeTree()

    print("\nPerforming multiple writes to trigger batch eviction...")
...
def interactive_test():
    ...
    while True:
        ...
        if choice == "1":
            addr = int(input("Enter block address: "))
...
if __name__ == "__main__":
    print("Range ORAM Testing Suite")
    print("------------------------")
